...
zero_weighting = (-torch.log(k.sum(1))).to(device)
...
plot_callback = PlotCallback(sdo_plot_dataset, model, prediction_dir, saturation_limit, device)
...
channel_weighting = torch.tensor([5, 1, 1, 0.1, 0.5, 5], dtype=torch.float32).view(1, 6, 1, 1).to(device)
...
for epoch in range(start_epoch, epochs):
    ...
    for iteration, sdo_image in enumerate(tqdm(sdo_train_loader, desc='Train')):
        ...
        optimizer.zero_grad()
        reconstructed_image, log_dem = model(sdo_image)

        loss = weigted_mse(reconstructed_image, sdo_image)
        second_order_regularization = (log_dem[:, :-2] - 2 * log_dem[:, 1:-1] + log_dem[:, 2:]).pow(2).mean()
        clipped_dem = torch.clip(log_dem - 20, min=0)
